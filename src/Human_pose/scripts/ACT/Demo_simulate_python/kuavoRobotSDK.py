...
class kuavo:
    ...
    def __init__(self, name):
        ...
        #修改增加最近的关节状态
        self.latest_JointPosition = None
        self.latest_RobotstatePosition = None
        ...
        # command
        self._JointState_sub = rospy.Subscriber("/kuavo_arm_traj", JointState, self.get_JointPosition_callback)
    ...
    def get_RobotState_callback(self, msg):
        self.latest_RobotstatePosition = msg.q
    ...
    def pub_kuavo_arm_traj(self, traj_jointstate):
        ...
        # print("当前打印的是kuavo的输出:", arm_traj_msg)
    ...
    def get_JointPosition(self):
        """ 获取机器人行走command """
        if self.latest_JointPosition is not None:
            return self.latest_JointPosition
        else:
            rospy.logwarn("No joint state data available.")

    def get_RobotstatePosition(self):
        """ 获取机器人行走command """
        if self.latest_RobotstatePosition is not None:
            return self.latest_RobotstatePosition
        else:
            rospy.logwarn("No RobotstatePosition state data available.")
    ...
    def set_arm_traj_position(self, joint_positions:list):
        ...
        if len(joint_positions) == self.arm_num:
            ...
            rospy.logerr("Invalid pub_kuavo_arm_trajnumber of joint positions provided.")
