...
from dynamic_biped.msg import QuaternionArray
from dynamic_biped.msg import handRotationEular
...
def send_data(sock, server_ip, server_port):
    ...
    # print(f"Get data {server_ip}:{server_port}  length is: {len(data)}")

    float_array = []
    points = []

    float_array = []  # Assuming float_array is defined somewhere above

    if len(data) < len(bone_names) * 4:
        return [],False
    # print(f"recevice {server_port} data length is {len(data)}")

    for i in range(0, len(data), 4):
        # print("i is: ", i, " is 12 mod: ", i % 12)
        # if(i% 12 == 0 and bone_names[int(i/12)] == pose_timestamp_name):
        #      print("Get pose_timestamp_name: ",struct.unpack('<f', data[i:i+4])[0])
        float_value = struct.unpack('<f', data[i:i+4])[0]
        # float_value = round(float_value, 5)  # Limiting the float value to 0.01
        float_array.append(float_value)

    left_hand_mark = Point3D(0xa5,0x5a,0xa5)
    right_hand_mark = Point3D(0x5a,0xa5,0x5a)

    result = 0

    # Convert float_array to a structured array of Point3D objects
    for i in range(0, len(float_array), 3):
        # print("convert folat to point i is:", i)
        if(i% 3 == 0 and bone_names[int(i/3)] == pose_timestamp_name):
            # print("Get pose_timestamp_name: ", float_array[i], " ", float_array[i+1], " ", float_array[i+2])
            ...
            if i + 2 < len(float_array):
                float_array[i] = (
                    float_array[i] if float_array[i] < 180 else float_array[i] - 360
                )
                float_array[i + 1] = (
                    float_array[i + 1]
                    if float_array[i + 1] < 180
                    else float_array[i + 1] - 360
                )
                float_array[i + 2] = (
                    float_array[i + 2]
                    if float_array[i + 2] < 180
                    else float_array[i + 2] - 360
                )
                point = Point3D(
                    float_array[i],
                    float_array[i + 1],
    ...
    # print(f" port is {server_port} first_mark is {first_mark} first points is : {points[0]}")
